  ...
  for (int i = 0; lflags[i]; i++) {
    ...
    while (it.next()) {
        if (it.isFolder()) {
            ...
            // TRACE/qmake/prl-load 1-1: Searches for .prl files prefixed with "lib" first, and
            // that unless the mkspec (like MSVC) has empty `QMAKE_PREFIX_STATICLIB`.
            QString libPrefix = project->first(ProKey("QMAKE_PREFIX_STATICLIB")).toQString();
            bool isWithPrefix = libPrefix.length() > 0;
            if (lib.isEmpty()) {
                goto found;
            }
        posRetry:
            for (QList<QMakeLocalFileName>::Iterator dir_it = dirs.begin();
                ...
                // Retry for local-path, and version.
                QString libBase = (*dir_it).local();
                libBase += '/';
                if (isWithPrefix) {
                    libBase += libPrefix;
                }
...
{
    // TRACE/qmake improve: aux-template may need some variables,
    // else would do:
    // ```
...
void Win32MakefileGenerator::writeStandardParts(QTextStream &t)
{
...
#else
    // TRACE/qmake/remote Makefile: ensured moc runs before building `SOURCES`,
    // to generate "*_remote.h" header, which's included by `SOURCES`.
    t << "SOURCES       = " << valList(escapeFilePaths(project->values("GENERATED_SOURCES")))
      << " " << valList(escapeFilePaths(project->values("SOURCES"))) << endl;
#endif

    // TRACE/qmake BugFix: we remove other entries from common directory-variables #2
    // for example, below `var("DESTDIR")` call would add #1 said space.
    ...
    // TRACE/qmake improve: aux should generate .res from .rc file (if set).
    t << endl;
    writeRcFilePart(t);
