...
/// @param dependencies Pass here an empty map, and get the needs map as result,
/// where each key is a module, and each value is what's needed-by said module.
///
/// @param dependees Pass here an empty map, and get the use-cases map as
/// result, where each key is a module, and each value is what uses said module,
/// like `dependees["core"] << "gui";` means QtCore is used by QtGui.
///
/// @param rootSet Any module that depends on nothing else.
        ...
        if (!dependencies.contains(item.toKey())) {
            ...
            if (depends.isEmpty()) {
                ...
                foreach (const ProString &dep, depends) {
                    const ProKey &depKey = dep.toKey();
                    dset.insert(depKey);
                    // See `dependees` in doc-comments.
                    dependees[depKey] << item;
                }
    ...
    case E_RESOLVE_DEPENDS:
        if (args.count() < 1 || args.count() > 5) {
            ...
            while (!rootSet.isEmpty()) {
                ...
                const ProString rootModule = *it;
                rootSet.erase(it);
                if ((func_t == E_RESOLVE_DEPENDS) || orgList.contains(rootModule))
                    ret.prepend(rootModule);
                // Queues the `useCase` of `rootModule` in `rootSet` (to be returned by this),
                // however, only if everything the said `useCase` itself needs is already queued,
                // which should result to sorting in order of what's needed first.
                foreach (const ProString &useCase, dependees[rootModule.toKey()]) {
                    QSet<ProKey> &useCaseNeeds = dependencies[useCase.toKey()];
                    useCaseNeeds.remove(rootModule.toKey());
                    if (useCaseNeeds.isEmpty())
                        rootSet.insert(first(ProKey(prefix + useCase + priosfx)).toInt(), useCase);
                }
            }

            // TRACE/qmake/built-in resolve_depends: now warns if any dependency remains missing.
            if (func_t == E_RESOLVE_DEPENDS) {
                QSet<ProKey> missing;
                auto it = dependencies.constBegin();
                ...
                if ( ! missing.isEmpty()) {
                    ...
                    int size = 0;
                    foreach(const ProKey &entry, missing) {
                        size += entry.size() + separator.size();
                    }
                    QString formattedList;
                    formattedList.reserve(size);
                    foreach(const ProKey &entry, missing) {
                        formattedList += entry;
                        formattedList += separator;
                    }
                    formattedList.chop(separator.size());

                    logicWarning(fL1S("Found recursion between dependencies: \"%1\"")
                                 .arg(formattedList));
                }
            }
        }
