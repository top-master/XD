...
uint qHash(const QSslCertificate &key, uint seed) Q_DECL_NOTHROW
{
    if (X509 * const x509 = key.d->x509) {
        ...
        // 1.1 made X509 opaque; the cached x509->sha1_hash field is gone, so
        // compute the SHA1 fingerprint directly via X509_digest.
        unsigned char hash[SHA_DIGEST_LENGTH];
        unsigned int len = 0;
        if (q_X509_digest(x509, q_EVP_sha1(), hash, &len) != 1)
            return seed;
        return qHashBits(hash, len, seed);
#else
...
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
        // 1.1: X509_get_version() returns the (0-based) version directly.
        d->versionString =
...
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
    // 1.1 made EVP_PKEY opaque; EVP_PKEY_base_id() replaces reaching into
    // pkey->type (which q_EVP_PKEY_type then reduced to the base id anyway).
    const int keyType = q_EVP_PKEY_base_id(pkey);
#else
    const int keyType = q_EVP_PKEY_type(pkey->type);
#endif
