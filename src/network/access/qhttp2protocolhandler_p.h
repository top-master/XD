...
/*!
    @file
    A clean-room HTTP/2 (RFC 7540) client protocol handler. It plugs into the
    same QAbstractProtocolHandler machinery the SPDY handler uses: requests are
    taken from the channel's multiplexed queue, each becomes an HTTP/2 stream,
    and the decoded responses are pushed back into their QHttpNetworkReply. No
    third-party code -- the framing and HPACK live in the http2/ helpers.
*/
...
private:
    ...
    /*!
        Client streams are odd, hence m_nextStreamID starts
        at 1 (set in the ctor).
    */
