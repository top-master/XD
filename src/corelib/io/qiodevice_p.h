...
public:
    ...
    public:
        ...
        inline QByteArray readAll() { return (m_buf ? m_buf->readAll() : QByteArray()); }
    ...
    {
        buffer.m_buf = (channel < readBuffers.size() ? &readBuffers[channel] : Q_NULLPTR);
        currentReadChannel = channel;
    }
    inline void setCurrentWriteChannel(int channel)
    {
        writeBuffer.m_buf = (channel < writeBuffers.size() ? &writeBuffers[channel] : Q_NULLPTR);
