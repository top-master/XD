...
class QScopedPointerLazy : public QScopedPointerLazyBase<T> {
    typedef void *QScopedPointerLazy:: *RestrictedBool;
...
public:
    ...
    {
        if (super::lazinessResolver().destroy(this, &d)) {
    ...
    inline T *data() const
    {
        if (super::isLoadPending()) {
            super::lazinessResolver().loadNow(
                    const_cast<Self *>(this), const_cast<void **>(&d));
        }
    ...
    inline bool reset(T *other = Q_NULLPTR)
    {
        ...
        if (super::lazinessResolver().replace(this, &d, reinterpret_cast<void **>(&other))) {
            Deleter::cleanup(oldD);
        }
        return static_cast<T *>(d) == other;
    }

    Q_ALWAYS_INLINE T *take()
    {
        return static_cast<T *>(super::lazinessResolver().take(this, &d));
    }

    Q_ALWAYS_INLINE bool swap(Self &other)
    {
        return super::lazinessResolver().swap(this, &d, &other.d);
    }
...
Q_INLINE_TEMPLATE void qLazyEventOverrideContext(QScopedPointerLazy<T> &newContext, QLazyEvent &event)
{
    ...
    QScopedPointerBase *base = QScopedPointerBase::get(&newContext);
