...
void tst_QFtp::init()
{
    ...
    if (setProxy) {
        ...
        if (proxyType == QNetworkProxy::Socks5Proxy) {
            // server-dummy's remapping SOCKS5 proxy (m_socksPort==1080) remaps the fleet host/port to the
            // loopback FTP listeners. Being all-loopback, it delivers the FTP 220 greeting in the same
            // read as the SOCKS CONNECT reply; QSocks5SocketEngine ungetChar's that greeting onto the
            // control socket, and it now surfaces on its own (see QIODevice::ungetChar's debounced
            // readyRead fix) -- so connectToHost no longer stalls ~61s for a WithoutSession client.
