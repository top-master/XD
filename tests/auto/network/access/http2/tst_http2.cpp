...
#include "../../../helpers/testenv.h" // bundled server-dummy fleet (h2 + files) + TestServer::port()
...
private:
    ...
    QRef<TestServer> m_fleet; // server-dummy fleet: HTTPS/h2 + serves rfc3252.txt
};
...
void tst_Http2::initTestCase()
{
    ...
    // The fleet serves /qtest/rfc3252.txt from the folder holding the reference file, so the
    // body round-trips byte-for-byte; HTTPS negotiates h2 via ALPN.
...
void tst_Http2::settingsAndNegotiation()
{
    ...
    QUrl url = qMove(m_fleet->url("/qtest/rfc3252.txt")); // https on the fleet's loopback stand-in port
