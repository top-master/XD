...
// QTestRule lives in testlib; server-dummy shares this header but links no testlib (it only uses the
// static TestServer::port()), so the rule aspect is compiled in only for actual test executables.
...
// A TestServer is also a QTestRule (in a test executable), so it can manage its own lifetime -- class-
// or case-scoped -- rather than being pinned by a `static`. See getServerForClass / getServerForCase.
...
#ifdef QT_TESTLIB_LIB
    // QTestRule scope (set by getServerForClass / getServerForCase). A case-scoped server stops itself
    // after each test function; every registered server is stopped at class end -- just before the rule
    // list is dropped, which then disposes it.
    void setLastsForCaseOnly(bool caseOnly) { m_lastsForCaseOnly = caseOnly; }
    void afterEach() Q_DECL_OVERRIDE { if (m_lastsForCaseOnly) stop(); }
    void afterClass() Q_DECL_OVERRIDE { stop(); }
#endif
...
private:
    ...
    bool m_secondSecure;
#ifdef QT_TESTLIB_LIB
    /// QTestRule scope: true = stop after each test function; false = live for the whole test class.
    bool m_lastsForCaseOnly;
#endif
};
