...
// QTestRule lives in testlib; server-dummy shares this header but does not link testlib.
...
#ifdef QT_TESTLIB_LIB
    /// Like getServer(), but the TestServer registers itself as a QTestRule and so keeps itself alive
    /// for the whole test class -- no `static` QRef needed at the call site. It stops itself at class
    /// end (TestServer::afterClass) and is disposed when the rule list is dropped.
    static QRef<TestServer> getServerForClass(TestServer::Type type, const QString &folder = QString())
    {
        QRef<TestServer> server = getServer(type, folder);
        QTestRule::add(server);
        return server;
    }

    /// Like getServerForClass(), but the server stops itself at the end of the current test function.
    static QRef<TestServer> getServerForCase(TestServer::Type type, const QString &folder = QString())
    {
        QRef<TestServer> server = getServer(type, folder);
        server->setLastsForCaseOnly(true);
        QTestRule::add(server);
        return server;
    }
#endif
