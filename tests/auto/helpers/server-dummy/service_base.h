...
protected:
    ...
    /// Appends one line to the process-wide server.log in the current working directory, tagged with
    /// the Unix time in milliseconds and a level. server-dummy runs one process per test-executable
    /// and each executable has its own working directory, so this log is naturally scoped to a single
    /// run without any per-test-case separation; the client harness keeps it only when a test fails
    /// and removes it on a clean run. log() records normal workflow steps (with timestamps, so a slow
    /// step shows up as a gap); logWarning() flags an anomaly worth surfacing.
    void log(const QString &message) const { writeServerLog("INFO", message); }
    void logWarning(const QString &message) const { writeServerLog("WARN", message); }
    ...
    void relayReadyRead()
    {
        ...
        if (s && peer) {
            ...
            peer->write(data);
            log(QLL("relay ") + QString::number(data.size()) + QLL(" bytes ")
                + (s->property("up").toBool() ? QLL("upstream->client") : QLL("client->upstream")));
        }
    }
    void relayDisconnected()
    {
        ...
        if (s && peer) {
            ...
            log(QLL("relay peer disconnected -- flushed tail and closed the other leg"));
        }
    }
    void onTunnelConnected()
    {
        ...
        log(QLL("tunnel upstream connected -- sent success reply to client"));
    }
    void onTunnelError()
    {
        QTcpSocket *up = qobject_cast<QTcpSocket *>(sender());
        ...
        logWarning(QLL("tunnel upstream failed before relaying: ") + up->errorString());
    }
...
private:
    /// Appends "<unix-ms> <LEVEL> <message>\n" to the one server.log shared by every service in this
    /// process (opened once, in the working directory, and kept open in append mode). server-dummy is
    /// single-threaded (one event loop), so no locking is needed. The leading Unix millisecond
    /// timestamp is what makes the log a profiling trace: consecutive lines whose stamps jump reveal
    /// exactly which step stalled.
    static void writeServerLog(const char *level, const QString &message)
    {
        static QFile logFile(QLL("server.log"));
        if (!logFile.isOpen()
            && !logFile.open(QIODevice::WriteOnly | QIODevice::Append | QIODevice::Text))
            return;
        logFile.write(QByteArray::number(QDateTime::currentMSecsSinceEpoch())
